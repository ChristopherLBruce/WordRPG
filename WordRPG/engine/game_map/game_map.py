""" Module for creating and manipulating Game_Map """
import os
...
class Game_Map(Table):
    ...
    **Keword Arguments:**
    ...
    def get_map(self, as_string=True, symbol=const.SETTINGS['symbol'],
        ...
        if as_string:
            return Game_Map.to_string(self)
        elif symbol is not True:
            return Game_Map.to_string(self, att='alpha')
        elif color is not True:
            return Game_Map.to_string(self, att='symbol')
        else:
            return Game_Map.to_string(self, att='char')


    def set_visibility(self):
        """ sets visiblity on Game_Map Tiles """
